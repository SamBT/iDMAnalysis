...
Bool_t RDFAnalysis::Process(TChain * chain) {
    ...
    auto takeMatchedDSAQuantityFloat_pad = [&](RVec<float> quant_dsa, size_t dsa_index_0, size_t best_muon_0, size_t dsa_index_1, size_t best_muon_1) {
        if (quant_dsa.size() == 0) return quant_dsa;
        RVec<float> matched{};
        if (dsa_index_0 != best_muon_0)
            matched.push_back(quant_dsa[dsa_index_0]);
        ...
        while (matched.size() < 2) {
            matched.push_back(-9999.0);
        }
        return matched;
    };

    auto takeUnmatchedDSAQuantityFloat_pad = [&](RVec<float> quant_dsa, size_t dsa_index_0, size_t best_muon_0, size_t dsa_index_1, size_t best_muon_1) {
        if (quant_dsa.size() == 0) return quant_dsa;
        RVec<float> unmatched{};
        if (dsa_index_0 == best_muon_0)
            unmatched.push_back(quant_dsa[dsa_index_0]);
        ...
        while (unmatched.size() < 2) {
            unmatched.push_back(-9999.0);
        }
        return unmatched;
    };

    auto takeSelectedMatchedGMQuantityFloat = [&](RVec<float> quant_gm, size_t dsa_index, size_t best_muon) {
        if (quant_gm.size() == 0) return -9999.f;
        if (dsa_index != best_muon && best_muon > 3) { 
            return quant_gm[best_muon-4];
        }
        else {
            return -9999.f;
        }
    ...
    auto df_wgts = df.
        ...
        Define("matched_dsa_dxy_padded", takeMatchedDSAQuantityFloat_pad, {"reco_dsa_dxy", "best_dsa_0", "best_muon_0", "best_dsa_1", "best_muon_1"}).
        Define("unmatched_dsa_dxy_padded", takeUnmatchedDSAQuantityFloat_pad, {"reco_dsa_dxy", "best_dsa_0", "best_muon_0", "best_dsa_1", "best_muon_1"}).
        Define("matched_gm_PFIso", takeMatchedGMQuantityFloat, {"reco_gm_PFIso","best_dsa_0", "best_muon_0", "best_dsa_1", "best_muon_1"}).
        Define("matched_gm_TrkIso", takeMatchedGMQuantityFloat, {"reco_gm_TrkIso","best_dsa_0", "best_muon_0", "best_dsa_1", "best_muon_1"}).
        Define("matched_gm_PFIso0", takeSelectedMatchedGMQuantityFloat, {"reco_gm_PFIso","best_dsa_0","best_muon_0"}).
        Define("matched_gm_PFIso1", takeSelectedMatchedGMQuantityFloat, {"reco_gm_PFIso","best_dsa_1","best_muon_1"}).
        Define("matched_gm_TrkIso0", takeSelectedMatchedGMQuantityFloat, {"reco_gm_TrkIso","best_dsa_0","best_muon_0"}).
        Define("matched_gm_TrkIso1", takeSelectedMatchedGMQuantityFloat, {"reco_gm_TrkIso","best_dsa_1","best_muon_1"}).
        //
